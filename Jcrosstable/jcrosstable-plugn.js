	...
	*/
	...
	function constructionColumnsHeader(tbody, firstTh) {			
		...
		$.each(allValuesColumns, function(index, value) {				
			...
			var th = $("<th>").attr({
				class:"columns", 
				colspan:1});
				//primeiro valor é sobre o índice da coluna para manipular o PIVOTING!
	...
	function constructionRowsHeader(tbody, total) {
		...
		$.each(opts.rows, function(i, value) {
			...
			th = $("<th>").attr({
				rowspan:1});	
				//primeiro valor é sobre o índice da coluna para manipular o PIVOTING! NO ID!	
	...
	$.extend($.fn, {
		jcrosstable: function(options) {
			...
			function drawCrossTable(div) {		
				...
				if(rows>=1) {
					//aqui entra somente se não tiver header em colunas
					if(!hasFirstTh) {
			...
			//função que faz o pivoting... se ele quiser
			function pivoting() {
			
				//pivoting começa aqui
				...
				$(".pivoting-droppable").droppable({				
					...
					drop: function(event, ui) {
						...
						//função para saber de qual eixo pertence o índice 0 para eixo das colunas e 1 para linhas
						...
						//função que retorna as opções do eixo que pertence o índice
						...
						//aqui remove o origem daonde ele veio.. pois não vai ter mais nd
